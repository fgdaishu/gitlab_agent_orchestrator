...
from .git_ops import commit_all, current_head, has_changes, prepare_issue_branch, push_branch, slugify
...
def process_job(job: Job, store: JobStore, client: GitLabClient) -> None:
    ...
    try:
        ...
        if sandbox:
            validation = sandbox.run_validation(settings.job_timeout_seconds)
            ...
            validation = run_validation(repo_dir, settings.job_timeout_seconds)
        ...
        client.add_issue_note(
            job.project_id,
            job.issue_iid,
            f"Agent job `{job.id}` completed.\n\n- Branch: `{branch}`\n- MR: !{mr_iid}\n- Validation: {validation.splitlines()[0]}",
        )
        ...
        if was_cancelled:
            ...
            store.mark_failed(job.id, error=str(exc), branch=branch, log_path=str(log_path) if log_path.exists() else None)
        try:
            labels = _current_labels(client, job)
            if was_cancelled:
